...
def get_voices():
    ...
    key = speech_key
    url = 'https://{}.cognitive.microsoft.com/api/texttospeech/v3.0/longaudiosynthesis/voices'.format(region)
    header = {
...
parser.add_argument('-locale', action="store", dest="locale", default=voice_language, help='the locale information like zh-CN/en-US')
parser.add_argument('-format', action="store", dest="format", default='riff-16khz-16bit-mono-pcm', help='the output audio format')
parser.add_argument('-key', action="store", dest="key", default=speech_key, help='the Speech service subscription key, like bb82464444c548dea4dce4376f3c7d26 ')
parser.add_argument('-region', action="store", dest="region", default=service_region, help='the region information, could be centralindia, canadacentral or uksouth, see the following link for a list of supported regions: https://docs.microsoft.com/en-us/azure/cognitive-services/speech-service/regions#speech-to-text-text-to-speech-and-translation')
...
parser.add_argument('-voiceId', action="store", nargs='+', default=voice_name, dest="voiceId", help='the id of the voice which used to synthesis')
